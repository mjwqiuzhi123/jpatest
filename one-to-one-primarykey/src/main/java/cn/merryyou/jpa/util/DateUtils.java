...
public class DateUtils {
    ...
    //计算某个日期至今相差年数
    public static Integer yearDateDiff(String startDate){
        ...
        double daysBetween=(new Date().getTime()-start.getTime()+1000000)/(60*60*24*1000);
        double d =Math.ceil(daysBetween/365);
        return (int)d;
    }

    /**
    * 得到指定日期所属周的第一天的开始 即 00：00：00 或最后一天的结束 23：59：59
    * @author      weikai01
    * @param dateStr 指定日期的字符串
    * @param dateTimeFormatter 指定日期字符串的格式 必须带时分
    * @param type 0-取第一天 1-代表取最后一天
    * @return
    * @exception
    * @date        2018-12-11 19:09
    */
    public static LocalDateTime getFirstOrLastDayOfWeek(String dateStr, String dateTimeFormatter,String type ){
        ...
        if("0".equals(type)) {
            return localDateTime.plusDays(1-dayOfWeek);
        }else {
           return localDateTime.plusDays(7-dayOfWeek).toLocalDate().atTime(23,59,59);
        }
    ...
    /**
     * 得到指定日期所属月的第一天的开始 即 00：00：00 或最后一天的结束23：59：59
     * @author      weikai01
     * @param dateStr 指定日期的字符串
     * @param dateTimeFormatter 指定日期字符串的格式 必须带小时
     * @param type 0-取第一天 1-代表取最后一天
     * @return
     * @exception
     * @date        2018-12-11 19:09
     */
    public static LocalDateTime getFirstOrLastDayOfMonth(String dateStr, String dateTimeFormatter,String type ){
        ...
        LocalDateTime firstDayOfManth = localDateTime.toLocalDate().atStartOfDay();
        if("0".equals(type)) {
            return firstDayOfManth.plusDays(1-dayOfMonth);
        }else {
            int leng = firstDayOfManth.getMonth().minLength();
            return firstDayOfManth.plusDays(leng-dayOfMonth).toLocalDate().atTime(23,59,59);
        }
    ...
    /**
     * 得到指定日期所属天的的开始 即 00：00：00 或结束23：59：59
     * @author      weikai01
     * @param dateStr 指定日期的字符串
     * @param dateTimeFormatter 指定日期字符串的格式 只有年月日
     * @param type 0-取开始 1-代表取结束
     * @return
     * @exception
     * @date        2018-12-11 19:09
     */
    public static LocalDateTime getFirstOrLastOfDay(String dateStr, String dateTimeFormatter,String type ){
        ...
        if("0".equals(type)){
         return localDate.atStartOfDay().toLocalDate().atTime(0,0,0);
    ...
    public static Timestamp defaultStrToTimeStamp(String dateStr) {
        return strToTimeStamp(dateStr, DATE_FORMAT);
    }

    /**
    * 查找指定日期的增加 类型为type 数量为number后 的日期：
     * 范围不精确到时分秒，而是每天的开始时间
     * 例如查询当前系统时间指定前前七天的日期
    * @author      weikai01
    * @param type 类型：year-年；month-月；week-周； day-日
    * @param number 指定数量
    * @return
    * @exception
    * @date        2018-12-23 15:23
    */
    public static LocalDateTime getLastNumberDate(LocalDateTime localDateTime,String type,Integer number){

        LocalDateTime rtnTime = null;

        switch (type){
            case "year" :
                rtnTime = localDateTime.toLocalDate().atStartOfDay().plusYears(number); break;
            case "month":
                rtnTime = localDateTime.toLocalDate().atStartOfDay().plusMonths(number);break;
            case "week":
                rtnTime = localDateTime.toLocalDate().atStartOfDay().plusWeeks(number);break;
            case "day":
                rtnTime = localDateTime.toLocalDate().atStartOfDay().plusDays(number);break;
             default: break;
        }
        return rtnTime;
    }
